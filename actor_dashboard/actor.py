...
목표: 입력된 배우들의 다차원 화제성 등급 정보와 전작 요약 정보를 바탕으로, 개별 배우의 단순 나열이 아니라 “배우 조합 구조”를 분석한다. 결과는 반드시 캐스팅 측면과 마케팅 측면으로 나누어, 캐스팅 관점의 강점·한계·보완 포인트와 마케팅 관점의 활용 방향·리스크·커뮤니케이션 포인트를 각각 실무형 코멘트로 정리한다.
...
<li>배역/조합 관점의 강점</li>
<li>메인/서브 상호작용 구조</li>
<li>한계 또는 보완 필요 지점</li>
</ul>
</div>
<div class="combo-section total">
<h3>[마케팅 측면]</h3>
<ul>
<li>초반 화제 선점 또는 홍보 활용 포인트</li>
<li>타깃 커뮤니케이션 방향</li>
<li>마케팅 리스크 또는 보완 포인트</li>
</ul>
</div>
</div>
...
4. [캐스팅 측면]과 [마케팅 측면]은 각각 3~5개의 li로 정리한다.
5. [캐스팅 측면]과 [마케팅 측면]에는 각각 한계 또는 보완 필요 지점을 1개 이상 포함한다.

=== [출력 규칙 상세] ===
1. HTML 태그 외의 텍스트는 출력하지 않는다.
2. Markdown 문법 사용 금지.
3. 코드블록 사용 금지.
4. h3 제목은 [메인 n인], [서브 n인], [캐스팅 측면], [마케팅 측면] 형식을 따른다.
...
8. 폭발력·안정성·기여도의 해석이 각각 살아 있고, 세 축이 캐스팅/마케팅 측면에서 어떻게 다르게 활용되는지가 드러난다."""
...
[data-testid="stHeader"] {display:none;}
        ...
        .similar-axis-grid {display:flex; flex-direction:column; gap:7px; margin-top:10px;}
        .similar-axis-row {display:flex; align-items:center; justify-content:space-between; gap:8px; font-size:0.82rem; line-height:1.25;}
        .similar-axis-name {min-width:48px; color:#667085; font-weight:800;}
        .similar-axis-badge {display:inline-block; min-width:42px; text-align:center; border-radius:999px; padding:4px 8px; font-weight:900;}
        .similar-axis-strong {background:#e7f8ef; color:#047857;}
        .similar-axis-same {background:#eef2ff; color:#3446a8;}
        .similar-axis-weak {background:#fff1f2; color:#be123c;}
        .similar-axis-diff {min-width:42px; text-align:right; color:#7b8495; font-size:0.78rem; font-weight:800;}
...
<div class='formula-box' style='background:#f7faff;'>
<b style='color:#0f172a;'>요약 산식</b> = 대표작 화제성 고점 + 저점 제외 후 나머지 작품의 상단 평균<br>
<span style='color:#64748b;'>한 줄 해석: 한 번 크게 터졌는지뿐 아니라, 다른 작품에서도 일정 체급을 유지했는지를 함께 봅니다.</span>
</div>
...
<div class='formula-box' style='background:#f7faff;'>
<b style='color:#0f172a;'>요약 산식</b> = 보정 평균 체급 + 히트 분산도 + 작품수 보정 + 대표작 고점 보조<br>
<span style='color:#64748b;'>한 줄 해석: 특정 한 작품에 몰린 성과인지, 여러 작품에서 반복적으로 재현된 성과인지 봅니다.</span>
</div>
<div class='formula-box'>
...
<div class='formula-box' style='background:#f7faff;'>
<b style='color:#0f172a;'>요약 산식</b> = 작품 내 화제성 점유 + 작품 내 랭킹 가중 + 작품 체급 보정<br>
<span style='color:#64748b;'>한 줄 해석: 작품이 뜬 것인지, 그 안에서 배우가 실제 중심축이었는지를 분리해서 봅니다.</span>
</div>
<div class='formula-box'>
...
def similar_score_actors(result_df: pd.DataFrame, row: pd.Series, top_n: int = 4) -> pd.DataFrame:
    ...
    idx = matches[0]
    upper_idx = list(range(max(0, idx - 2), idx))
    lower_idx = list(range(idx + 1, min(len(sorted_df), idx + 3)))

    selected_idx = upper_idx + lower_idx
    need = top_n - len(selected_idx)

    if need > 0:
        # 위쪽 후보가 부족하면 아래쪽에서 더 채우고, 아래쪽 후보가 부족하면 위쪽에서 더 채웁니다.
        extra_lower = [i for i in range(idx + 1, len(sorted_df)) if i not in selected_idx]
        extra_upper = [i for i in range(idx - 1, -1, -1) if i not in selected_idx]
        if len(upper_idx) < 2:
            selected_idx.extend(extra_lower[:need])
        else:
            selected_idx.extend(extra_upper[:need])

    sim = sorted_df.loc[selected_idx[:top_n]].copy()
    sim["score_gap"] = sim["합산점수"] - float(row["합산점수"])
    sim["relative_position"] = np.where(sim["score_gap"] > 0, "위", "아래")
    return sim


def axis_strength_html(candidate: pd.Series, base_row: pd.Series, axis_label: str, pct_col: str, grade_col: str) -> str:
    cand_pct = float(candidate.get(pct_col, 0) or 0)
    base_pct = float(base_row.get(pct_col, 0) or 0)
    diff = cand_pct - base_pct
    if diff > 0.015:
        klass = "similar-axis-strong"
        label = "강"
    elif diff < -0.015:
        klass = "similar-axis-weak"
        label = "약"
    else:
        klass = "similar-axis-same"
        label = "유사"
    sign = "+" if diff > 0 else ""
    return (
        f"<div class='similar-axis-row'>"
        f"<span class='similar-axis-name'>{axis_label}</span>"
        f"<span class='similar-axis-badge {klass}'>{candidate.get(grade_col, '')} · {label}</span>"
        f"<span class='similar-axis-diff'>{sign}{diff * 100:.0f}p</span>"
        f"</div>"
    )
...
def similar_actor_card_html(candidate: pd.Series, base_row: pd.Series) -> str:
    ...
    axes_html = "".join([
        ...
        axis_strength_html(candidate, base_row, "기여", "기여백분율", "기여도등급"),
    ])
    return f"""
    <div class='mini-card' style='min-height:178px;'>
        <div>{actor_link_html(candidate['배우'], extra_style='display:block;')}</div>
        <div class='actor-sub'>합산등급 <b>{candidate['합산등급']}</b> · {format_score(candidate['합산점수'])} <span style='color:#94a3b8;'>({gap_txt})</span></div>
        <div class='similar-axis-grid'>{axes_html}</div>
    </div>
    """
...
def render_detail(raw_df: pd.DataFrame, result_df: pd.DataFrame):
    ...
    if sim.empty:
        ...
        for i, (_, r) in enumerate(sim.iterrows()):
            with cols[i % 4]:
                st.markdown(similar_actor_card_html(r, row), unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
...
def render_compare(raw_df: pd.DataFrame, result_df: pd.DataFrame):
    ...
    tab_ai_explore, tab_ai_compare, tab_ai_partner, tab_ai_combo = st.tabs(["탐색하기", "비교하기", "상대배우 찾기", "조합분석"])
...
def render_rising_star_placeholder():
    st.markdown("<div class='section-title'>라이징스타 감지기</div>", unsafe_allow_html=True)
    st.markdown(
        """
        <div class='card' style='min-height:220px; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;'>
            <div class='summary-big' style='font-size:1.7rem;'>추후 업데이트 예정</div>
            <div class='summary-sub' style='margin-top:12px;'>라이징스타 감지 로직은 이후 업데이트에서 제공됩니다.</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def main():
    ...
    if page == "OVERVIEW":
        ...
        render_rising_star_placeholder()
    else:
        render_reference()
